...
// 2. bookNo destructor.
// the synthesized destructor for Sales_data accum destroys the data member bookNo (string). 
// as for units_sold (unsigned), and revenue (double), they are built-in types and have no destructor. 

// 3. item1 destructor.

// 4. bookNo destructor. 
// the synthesized destructor for Sales_data item1 destroys the data member bookNo (string). 
// as for units_sold (unsigned), and revenue (double), they are built-in types and have no destructor. 

// 5. item2 destructor.

// 6. the synthesized destructor for Sales_data item2 destroys the data member bookNo (string). 
// as for units_sold (unsigned), and revenue (double), they are built-in types and have no destructor. 
