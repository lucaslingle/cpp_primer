/*
In the `OrQuery::eval` function, what would happen if its `rhs` member 
returned an empty set? What if its `lhs` member did so? 
What if both `rhs` and `lhs` returned empty sets?
*/

// if one of them returned empty sets, the initial set would be empty
// if the other of them did, the inserted elements would be none
// if both returned empty sets, the `QueryResult` returned by eval 
//     would have an empty linenos member.
